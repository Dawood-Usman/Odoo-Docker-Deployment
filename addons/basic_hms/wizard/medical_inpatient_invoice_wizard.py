...
class medical_inpatient_invoice_wizard(models.TransientModel):
    _name = "medical.inpatient.invoice.wizard"
    _description= 'medical inpatient invoice wizard'
    ...
    def create_invoice(self):
        ...
        for active_id in active_ids:
            ...
            if lab_req.is_invoiced  == True:
                ...
                if lab_req.product_id.id:
                    invoice_line_account_id = lab_req.product_id.property_account_income_id.id or lab_req.product_id.categ_id.property_account_income_categ_id.id or False
                if not invoice_line_account_id:
                    inc_acc = ir_property_obj.get('property_account_income_categ_id', 'product.category')
                if not invoice_line_account_id:
                    raise UserError(
                        _('There is no income account defined for this product: "%s". You may have to install a chart of account from Accounting app, settings menu.') %
                        (lab_req.product_id.name,))

                tax_ids = []
                taxes = lab_req.product_id.taxes_id.filtered(lambda r: not lab_req.product_id.company_id or r.company_id == lab_req.product_id.company_id)
                tax_ids = taxes.ids
                invoice_line_vals = {
                    'name': lab_req.product_id.name or '',
                    'account_id': invoice_line_account_id,
                    'price_unit':lab_req.product_id.standard_price,
                    'product_uom_id':lab_req.product_id.uom_id.id,
                    'quantity': 1,
                    'product_id':lab_req.product_id.id,
                }
